...
[source,bash]
----
git clone https://gitlab.univ-nantes.fr/${USER}/projet-202#.git
cd projet-2021
npm install
----
...
.Cette instruction précédente  récupère les changements et les fusionne avec votre divergence (faire chaque fois que l'in souhaite récupérer les changements)
[source,sh]
--
...
Vous devez mettre en oeuvre la validation des déplacements des pions dans mes autres cases: grises (Dame), jaunes (Cavalier), bleues (Fou) et rouges (Tour). 
...
Vous devez donc parcourir le module `move-validation` et implémenter les fonctions de validation contenant le commentaire "`// {hash}TODO:`". 
...
export class TestRookMoves {
    ...
    beforeEach(){
        ...
        // Place un pion sur la case B4 d'un échiquier vide:
    ...
    testCanMoveHorizontally() {
        // Les variable "move.B4_H4" et "moveB4_A4" ont été créées au début 
        // du module pour simplifier le code des tests.
        // Le déplacement doit être possible:

        Expect(isPossible.rookMove(chessboard, moveB4_H4)).toBeTruthy();
        Expect(isPossible.rookMove(chessboard, moveB4_A4)).toBeTruthy();
    }
}
----
...
export class TestRookMoves {
    ...
    testCanMoveVertically() {
        Expect(isPossible.rookMove(chessboard, moveB4_B8)).toBeTruthy();
        Expect(isPossible.rookMove(chessboard, moveB4_B1)).toBeTruthy();
    }
}
----
